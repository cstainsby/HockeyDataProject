...
def apriori(table, minsup, minconf):
    """
    The base apriori algorithem, I broke it down into many smaller functions
    because it looks nicer, and i can clearly see each step
    Args:
        table: The table of all the data
        minsup: minimum support
        minconf: minimum confidence

    Returns:
        The rules generated by the algorithem
    """

    supported_itemsets = []

    I = compute_unique_values(table)
    supported_itemsets.append(support(I, table, minsup))

    k = 2
    while(len(supported_itemsets[k-2]) != 0):
        Ck = make_ck(supported_itemsets[k-2])
        supported = support(Ck, table, minsup)
        k += 1
        supported_itemsets.append(supported)

    all_supported = []
    for index in range(len(supported_itemsets)):
        if index > 0 and index < k-1:
            for val in supported_itemsets[index]:
                all_supported.append(val)


    rules = generate_apriori_rules(all_supported, table, minconf)

    return rules


def calc_lift(rule, table):
    """
    Calculates the lift of the rule
    Args:
        rule: the rule in question
        table: the base table of values

    Returns:
        the lift of the rule
    """
    total_support = support_calc(rule, table)

    rhs_counts= 0
    for row in table:
        if check_val_in_val(row, rule["THEN"]):
            rhs_counts += 1

    lhs_counts = 0
    for row in table:
        if check_val_in_val(row, rule["IF"]):
            lhs_counts += 1

    return total_support / ((lhs_counts/len(table)) * (rhs_counts/len(table)))


def support_calc(rule, table):
    """
    calculates support of the rule
    Args:
        rule: the rule in question
        table: the base table of values
    ...
    for val in rule["IF"]:
        union.append(val)
    for val in rule["THEN"]:
        union.append(val)

    union_counts = 0
    for row in table:
        if check_val_in_val(row, union):
            union_counts += 1

    return union_counts / len(table)


def generate_apriori_rules(supported_itemsets, table, minconf):
    """
    generates the apriroi rules for given itemsets
    Args:
        supported_itemsets: the item sets in question
        table: base table of values
        minconf: minimum confidence

    Returns:
    the list of rule dictionarys
    """
    rules = []
    for itemset in supported_itemsets:
        for rhs_size in range(len(itemset)-1):
            for x in range(rhs_size+1):
                if x == 0:
                    for val in itemset:
                        rule = {"IF":[], "THEN":[val]}
                        for item in itemset:
                            if item != val:
                                rule["IF"].append(item)
                        rule_conf = confidence(rule["IF"], rule["THEN"], table)
                        if rule_conf >= minconf:
                            if rule not in rules:
                                rules.append(rule)
                else:
                    combinations = itertools.combinations(itemset, x+1)
                    items = []
    ...
    for rule in rules:
        rule["Confidence"] = confidence(rule["IF"], rule["THEN"], table)
        rule["Support"] = support_calc(rule, table)
        rule["Lift"] = calc_lift(rule, table)

    return rules


def confidence(lhs, rhs, table):
    """
    calculates confidence on a rule
    Args:
        lhs: left hand side
        rhs: right hand side
        table: base table of values
    ...
    lhs_counts = 0
    for row in table:
        if check_val_in_val(row, lhs):
            lhs_counts += 1

    return union_counts/lhs_counts


def combine(item1, item2):
    """
    Combines 2 different lists into one list no repeats
    Args:
        item1: list 1
        item2: list 2

    Returns:
        a combined list of the 2 items with no repeats
    """
    new_piece = []
    for val in item1:
        new_piece.append(val)
    for val in item2:
        if val not in new_piece:
            new_piece.append(val)
...
def make_ck(itemset):
    """
    Makes the Ck for a given itemset by checking if all but the rhs is equal and
    then uses the combine method to add them together
    Args:
    ...
    for item in itemset:
        for other in itemset:
            if item != other:
                if item[:-1] == other[:-1]:
                    combined = combine(item, other)
                    if combined not in Ck:
                        Ck.append(combined)
    return Ck
...
def compute_unique_values(table):
    ...
    Returns:
        a list of the unique values within the table
    """
    unique = []
    for row in table:
        for value in row:
...
def support(I, table, minsup):
    """
    Calculates the support of a given subset
    Args:
        I: the subset list in question
        table: the base table
        minsup: the minimum support required to keep

    Returns:
        a list of supported subsets
    """
    inorder_list = []
    for x in range(len(I)):
        inorder_list.append(0)
        for row in table:
            if type(I[x]) != list:
                if check_val_in_val(row, [I[x]]):
                    inorder_list[x] += 1
            else:
                if check_val_in_val(row, I[x]):
                    inorder_list[x] += 1
    ...
    supported_list = []
    for x in range(len(I)):
        if inorder_list[x]/len(table) >= minsup:
            supported_list.append(I[x])
    return supported_list


def check_val_in_val(val1, val2):
    """
    a function to check if a smaller subset is within a bigger piece
    Args:
        val1: bigger piece
        val2: checked to be within val 1

    Returns:
        returns true if val2 is in val1, false otherwise
...
def print_rules(rules):
    """
    A function to print rules
    Args:
        rules: the list of rules

    Returns:
        nothing
        prints the rules in correct format
    """
    rule_table = []
    for x in range(len(rules)):
        ...
        for val in rhs:
            if then_string != "THEN":
                ...
                then_string += " " + val
        rule_table.append([x+1,(rule_string + " " + then_string), rules[x]["Support"], rules[x]["Confidence"], rules[x]["Lift"]])
    print("Rule Table\n" + tabulate(rule_table, headers=["#", "RULE", "SUPPORT", "CONFIDENCE", "LIFT"]))
